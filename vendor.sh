#!/usr/bin/env bash
# Updates the vendored Smarty SDK and summary library inside the plugin directory.
# Usage: ./vendor.sh
# After running, commit the updated vendored directories.
set -euo pipefail
...
# Remove old vendored summary artifacts
rm -rf "$PLUGIN_DIR/summary"
rm -rf "$PLUGIN_DIR"/summary-*.dist-info

# Install libraries without transitive deps — requests is bundled with QGIS's Python
...
echo "Done. Commit the updated vendored directories."
